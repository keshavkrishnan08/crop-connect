    ...
    return (
        <div className="mb-6 overflow-hidden rounded-3xl border border-brand-200 bg-gradient-to-br from-brand-50/80 via-canvas-soft to-canvas-soft shadow-card transition-shadow duration-200 hover:shadow-lift">
            <div className="flex items-center gap-2.5 border-b border-brand-100/70 px-5 py-3">
                <AgentAvatar size={26} />
                <p className="text-sm font-medium text-ink">{AGENT_NAME} drafted this week's special</p>
            ...
            <div className={`grid gap-5 p-5 sm:grid-cols-[1.5fr_1fr] sm:p-6 ${spin ? "animate-fade-up" : ""}`}>
                <div>
                    ...
                    <div className="mt-4 flex items-center gap-6">
                        <div><p className="text-2xs uppercase tracking-wide text-ink-faint">Menu price</p><p className="font-display text-2xl text-ink tnum">{usd(sp.price)}</p></div>
                        <div><p className="flex items-center gap-1 text-2xs uppercase tracking-wide text-ink-faint"><MarginUp size={11} /> Adds about</p><p className="font-display text-2xl text-brand-600 tnum">{usd(monthly, { compact: monthly > 9999 })}<span className="text-sm text-ink-muted">/mo</span></p></div>
                    </div>
                ...
                <Photo q={`${sp.crop} dish plated restaurant`} seed={200 + (i % WEEK_SPECIALS.length)} alt={sp.dishName} className="min-h-[190px]" />
            </div>
        </div>
    );
}
